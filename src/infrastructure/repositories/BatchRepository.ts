...
export class BatchRepository {
  ...
  async getExistence(line: SaleLine): Promise<any | null> {
    const productXSupplier = await this.client.productxsupplier.findUnique({
      where: {product_id_supplier_id: {product_id: line.snapshot.product.snapshot.id as number, supplier_id: line.snapshot.supplierId}}
    });
  ...
  async compromiseProductsByLines(saleLines: SaleLine[]) {
    const compromisedSaleLines = await Promise.all(
        ...
        const databaseBatches = await this.client.batch.findMany({
          ...
          select: {actualStock: true, compromised: true, arriveDate: true, batch_id: true, supplier_id: true, product_id: true}
        });
        ...
        for (const batch of selectedBatches) {
          await this.client.batch.update({
            where: {product_id_supplier_id_batch_id: {batch_id: batch.batch_id, product_id: batch.product_id, supplier_id: batch.supplier_id}},
            data: {
